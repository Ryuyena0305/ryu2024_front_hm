/*
    ...
    HTML 마크업 이벤트 함수
        1. onclick : 지정한 마크업을 클릭했을 때 js코드
...
//[2] '등록함수' 함수 정의 : [ 등록 ] 버튼 클릭시 input에 입력된 값을 배열에 저장하는 함수
    ...
    function 등록함수(){
    // - DOM 객체 { document} : HTML  조작 가능한 객체
...
} //->함수가 종료되면서 입력받은 'data'변수는 사라진다.

//[3] '출력함수' : 함수 정의 : 배열(이름명단)의 변화(등록버튼을 눌렀을 때)가 있을 때 화면(HTML) 새로고침(출력)하는 함수
